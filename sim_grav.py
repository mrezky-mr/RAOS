...
print("sim_grav.py starts running at ", start)
print('-----')
        ...
        for q in range(smr.N - smr.N_active):
            ...
            if saved_col == False:
                ...
                dict = {"t": tis, "obj_e": ecc, "obj_a": asm, "obj_i": inc, 
                        ...
                        "erg_re": err, "anm_a": anm, "anm_e": anr, "res_ma": amm, 
                        "res_me": ecm, "res_mo": aom, "res_va": amv, "res_ve": ecv, 
                        "res_vo": aov}
                df = pd.DataFrame(dict)
                if n == 0:
                    df.to_csv('grav_' + start + '_data_' + labl + '.csv')
                else:
                    df.to_csv('grav_' + start + '_data_' + labl + '.csv', mode='a', header=False)
...
di.to_csv('grav_' + start + '_init.csv')

'''INITIATE AND WRITE THE SIMULATION LOGS'''
file = open("grav_" + start + "_summary.txt","w")
